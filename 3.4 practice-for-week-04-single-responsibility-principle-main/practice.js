...
function bakePie(pieNumber){
  return `Baked pie ${pieNumber + 1}!`;
}

function calculateCost(recipe, pieQuantity) {
  const costOfPie = recipe.reduce((prev, current) => {
    if (current.name !== "pie crust") {
      return prev + current.cost;
    } else {
      return prev;
    }
...
function calculateRevenue(totalCost, profitMargin = 1.2){
...
function bakeAndSellPies(pieType, pieQuantity, profitMargin) {
  ...
  for (let i = 0; i < pieQuantity; i++) {
    ...
    console.log(bakePie(i));
  }

  const totalCost = calculateCost(recipe, pieQuantity);
  const costPerPie = (totalCost / pieQuantity).toFixed(2);
